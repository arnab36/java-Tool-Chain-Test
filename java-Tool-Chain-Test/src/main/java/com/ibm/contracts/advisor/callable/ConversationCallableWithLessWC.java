...
public class ConversationCallableWithLessWC implements Constants, Callable<ClassificationVO>{
	...
	public ConversationCallableWithLessWC(String sentence,ConversationService service,int threadNum) {
	...
	public ClassificationVO callAsync() throws Exception {
		...
		try{
		  ...
		  for(int k=0;k<3;k++){
			...
			System.out.println(e.toString()+" inside processing the conv result");
		...
		if(APIcallable ==false)
			executed=true;
		ClassificationVO cVo=new ClassificationVO(this.threadNum,tmp.toString(),executed);
		return cVo;
	}
