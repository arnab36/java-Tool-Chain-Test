...
function start_conversation(){
	...
	if(conversation_start_flag == 0) {
		...
		$.ajax({
			...
			data: { 
						param_1: json_data_1,
						param_2: conversation_start_flag					
					},					
			success: function (message) {
				var dNow = new Date();				
				time = dNow.getHours() + ':' + dNow.getMinutes() + ':' + dNow.getSeconds();
				$("#messageList").append( '<ul id="watsonChat"><img src="static/img/cisco-chat-logo.png" style="height:38px;width:38px;"> ' + message["jsonResponse"]+ '</ul>');
				$("#messageList").scrollTop($("#messageList")[0].scrollHeight);
				$("#messageList").removeClass("w3-light-gray");
				$("#messageText").prop('disabled', false);
				$("#messageText").focus();
				console.log("message event triggered for: " + message["jsonResponse"]);	
				console.log("message event triggered for: " + message["data"]);					
               ...
               // $("#messageList").append( '<ul><font color="red">Me: </font>' +$("#messageText").val()+ '</ul>');
				...
				$.ajax({
					...
					success: function (message) {
						var dNow = new Date();
						time = dNow.getHours() + ':' + dNow.getMinutes() + ':' + dNow.getSeconds();
						$("#messageList").append( '<ul id="responseChat"><img src="static/img/cisco-chat-logo.png" style="height:38px;width:38px;"> ' +message["jsonResponse"]+ '</ul>');
						$("#messageList").scrollTop($("#messageList")[0].scrollHeight);
						$("#messageList").removeClass("w3-light-gray");
						$("#messageText").prop('disabled', false);
						$("#messageText").focus();
